  ...
  module Utils
    class HashUtil
      class << self
        ...
        def convert(object, keys = [], key_func: nil, value_func: nil, object_func: nil) # {{{
          return if object_func && object_func.call(object, keys) == false
          if object.is_a?(Hash)
            object.each_with_object({}) do |(k, v), obj|
              key = key_func ? key_func.call(k) : k
              value = convert(v, keys + [k], key_func: key_func, value_func: value_func, object_func: object_func)
              next if value.nil?
              obj[key] = value
            end
          elsif object.is_a?(Array)
            object.each_with_object([]) do |v, obj|
              value = convert(v, keys, key_func: key_func, value_func: value_func, object_func: object_func)
              next if value.nil?
              obj << value
            end
          else
